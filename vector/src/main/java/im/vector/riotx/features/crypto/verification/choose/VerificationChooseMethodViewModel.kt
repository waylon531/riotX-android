...
class VerificationChooseMethodViewModel @AssistedInject constructor(
    ...
    override fun verificationRequestUpdated(pr: PendingVerificationRequest) = withState { state ->
        val pvr = session.getCryptoService().getVerificationService().getExistingVerificationRequest(state.otherUserId, state.transactionId)
    ...
    companion object : MvRxViewModelFactory<VerificationChooseMethodViewModel, VerificationChooseMethodViewState> {
        ...
        override fun initialState(viewModelContext: ViewModelContext): VerificationChooseMethodViewState? {
            ...
            val pvr = session.getCryptoService().getVerificationService().getExistingVerificationRequest(args.otherUserId, args.verificationId)

            // Get the QR code now, because transaction is already created, so transactionCreated() will not be called
            val qrCodeVerificationTransaction = session.getCryptoService().getVerificationService().getExistingTransaction(args.otherUserId, args.verificationId ?: "")
