...
class DefaultNavigator @Inject constructor(
    ...
    override fun performDeviceVerification(context: Context, otherUserId: String, sasTransationId: String) {
        ...
        val tx = session.getCryptoService().getVerificationService().getExistingTransaction(otherUserId, sasTransationId) ?: return
    ...
    override fun requestSessionVerification(context: Context) {
        val session = sessionHolder.getSafeActiveSession() ?: return
        val pr = session.getCryptoService().getVerificationService().requestKeyVerification(
                listOf(VerificationMethod.SAS, VerificationMethod.QR_CODE_SCAN, VerificationMethod.QR_CODE_SHOW),
                session.myUserId,
                session.getCryptoService().getUserDevices(session.myUserId).map { it.deviceId })
        if (context is VectorBaseActivity) {
            VerificationBottomSheet.withArgs(
                    roomId = null,
