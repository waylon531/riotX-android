...
import im.vector.matrix.android.internal.db.auth.Pending_session
...
internal class SqlitePendingSessionMapper @Inject constructor(moshi: Moshi) {
    ...
    fun map(entity: Pending_session?): PendingSessionData? {
        if (entity == null) {
            return null
        }
    ...
    fun map(sessionData: PendingSessionData?): Pending_session? {
        if (sessionData == null) {
            return null
        }
